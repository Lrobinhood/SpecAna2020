...
void CPidFilter::SetPid(u16 u16PID)
{
    if (m_u16PID != u16PID)
    {
        ...
        _snprintf_s(record_name, 32, "PID_0x%x.log", m_u16PID);
        
        m_fpRecord = new CRecordFile(record_name);
    }
}
	...
	if (m_u16PID == u16PID)
	{
        ...
        if (m_isRecord)
        {
            if (NULL != m_fpRecord)
            {
                ...
                _snprintf_s(record_log, 64, "%8x: %2x %2x %2x %2x", szStreamOffset, pu8Data[0], pu8Data[1], pu8Data[2], pu8Data[3]);
...
void CPidFilter::RegisterNotify(u32 Key,  CFilterBase &Filter)
{
...
void CPidFilter::UnRegisterNotify(u32 Key)
{
